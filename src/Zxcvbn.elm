...
The results returned by this package should in all cases be exactly identical to those returned by the reference (javascript) implementation. If you do not require exactly identical results, consider using `ZxcvbnPlus` instead, which is a slightly more optimized and "Elm"-y implementation.
...
This package exposes exactly one function, `zxcvbn`, which requires no configuration of any kind and may be simply called with a password (and optional list of additional patterns).
...
{-| `zxcvbn` takes two arguments, a `List String` of user patterns and a `String` password, and returns a result object of type `ZxcvbnResult`.

The user patterns argument (which may be an empty list) is an array of strings that `zxcvbn` will treat as an extra dictionary. This can be whatever list of strings you like, but is meant for user inputs from other fields of the form, like name and email. That way a password that includes a user's personal information can be heavily penalized. This list is also good for site-specific vocabulary — Acme Brick Co. might want to include `["acme", "brick", "acmebrick", etc]`. This list is ordered in increasing rank, which the first element of the list being the highest rank (most common). Entries with higher ranks are more strongly penalized. However, unless a truly massive list is used, the ordering is unlikely to be relevant.
...
{-| `ZxcvbnResult` is returned by `zxcvbn` and holds the results of the password strength check. It has the following fields:
  ...
  - `sequence` -- The list of patterns that zxcvbn based the guess calculation on. Rather complicated and typically unnecessary.
